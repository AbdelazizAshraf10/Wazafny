...
function ModalApply({
  ...
  useEffect(() => {
    const fetchCountriesAndCities = async () => {
      try {
        ...
        setSubmissionError("Failed to load countries. Please try again later.");
      }
    };
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      Object.assign(newErrors, currentStepErrors);
    } else if (step === 2) {
      if (!formData.resume) {
  ...
  const handleSubmit = async () => {
    ...
    formDataToSend.append("country", formData.country); // Send country name as per sample data
          ...
          <div className={`${stepPaddingClasses} py-4 space-y-4`}>
            ...
            <div>
              ...
              <PhoneInput
                ...
                inputClass="!border-gray-300 !rounded-md !text-sm !text-gray-700"
            ...
            <div className={`border-2 ${errors.resume ? 'border-red-500' : 'border-dashed border-gray-400'} rounded-lg p-6 text-center min-h-[160px] flex flex-col items-center justify-center transition-colors duration-200 hover:border-[#6A0DAD]`}>
              {formData.resume ? (
                ...
                <label className="cursor-pointer flex flex-col items-center text-gray-600 hover:text-[#6A0DAD]">
                  <img src={cv} alt="CV Icon" className="w-12 h-12 mb-2 text-[#6A0DAD]" />
