...
function Resume({ userRole, initialResume }) {
  ...
  const [resumeUrl, setResumeUrl] = useState(""); // Store the resume URL
  const [dragging, setDragging] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");
  const [apiError, setApiError] = useState(""); // For API errors
  const [isNewFile, setIsNewFile] = useState(false); // Track if a new file is uploaded
  ...
  useEffect(() => {
    if (initialResume) {
      ...
      // If initialResume is a URL, we can create a pseudo-file object for display
      const fileName = initialResume.split("/").pop();
      setFile({ name: fileName });
      setIsNewFile(false); // Initial file is not considered new
    }
  }, [initialResume]);
  ...
  const handleFileChange = (event) => {
    ...
    if (
      ...
      setIsNewFile(false); // Reset new file status
    }
  };
